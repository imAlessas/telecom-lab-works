...
figure(1);
plot(symNoi, 'b.');
hold on, plot(mapQAM16, 'rx'), hold off;
xlim([-4, 4]), ylim([-4, 4]);
xlabel('Real Part'), ylabel('Immaginary Part');
axis square;

% ----------------------------------------------------------------------             
% Perform demodulation by finding closest ideal signal from constellation and
% convert decimal symbols into binary codewords. This script is provided.
% Feel free to study its contents and evaluate its operation.
demodData

% ----------------------------------------------------------------------
% Count number of errors by comparing vectors binDt and binDmd.
% Use '~=' logical operator for comparison and count number of mismatches
